...
class V2AgentGraphTestCase(TestCase):
    ...
    @patch('core.agents.planner.PlannerAgent.generate_plan')
    @patch('core.agents.research_agent.ResearchAgent.execute')
    @patch('core.agents.job_reasoning_agent.JobReasoningAgent.execute')
...
class AgentMemoryTestCase(TestCase):
    ...
    def test_memory_injection_and_extraction(self, mock_generate):
        ...
        # Test Injection
        from core.agents.v2_graph import memory_injection_node
        ...
        from core.agents.v2_graph import memory_extraction_node
        extraction_state = {
            "messages": [
...
class WebSocketStreamingTestCase(TransactionTestCase):
    ...
    async def test_stream_agent_update_sends_websocket_payload(self):
        ...
        from core.agents.v2_graph import stream_agent_update
...
class ConversationAPIEndpointsTestCase(TestCase):
    def setUp(self):
        from memory.models import UserProfile, Conversation, Message
        self.user = UserProfile.objects.create(username="default_user", email="default@example.com")
...
class IntelligentRouterTestCase(TestCase):
    ...
    def test_fallback_flow(self):
        ...
        with patch('core.llm.router.GroqAdapter.generate') as mock_groq, \
             patch('core.llm.router.OllamaAdapter.generate') as mock_ollama:
    ...
    @patch('core.llm.router.GeminiAdapter.generate')
    ...
    @patch('core.llm.router.GroqAdapter.generate')
